...
function main()
    ...
    PR, Spread = data_amarel(L, fib(11),  600000, rank, Nproc, 0.0005, 1, ARGS[1], ARGS[2], init_FFT(L, 1:2), init_IFFT(L, 1:2), kin_spin_matrix(zeros(ComplexF64, L, L, 2, 2), L, 0.0005))
    sum_PR = MPI.Reduce(PR, +, 0, comm)
    sum_Spread = MPI.Reduce(Spread, +, 0, comm)

    if rank == 0
        average_PR = sum_PR/Nproc
        save("/scratch/bomeisl/6-1-20/PR_L=$(L)_$(del_t)_$(t)_gauss-1_avg/Spread_L=$(L)_g=$(ARGS[1])_W=$(ARGS[2])_gauss-1_avg.jld", "data", average)
        average_Spread = sum_Spread/Nproc
        save("/scratch/bomeisl/6-1-20/Spread_L=$(L)_$(del_t)_$(t)_gauss-1_avg/Spread_L=$(L)_g=$(ARGS[1])_W=$(ARGS[2])_gauss-1_avg.jld", "data", average)
    end
...
function data_amarel(L, m, t, rank, Nproc, del_t, thin_fac, g, W, FFT, IFFT, spin_matrix)
    ...
    return PR_Spread_fast(psi_guess_array(x_dummy(L), L), t, del_t, spin_matrix, ARGS[1], FFT, IFFT, L, pot_matrix_QP, ARGS[2])
